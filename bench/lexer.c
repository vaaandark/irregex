...
regex_t compiled_regex[rule_num];
...
int main(int argc, char *argv[]) {
    ...
    for (i = 0; i < rule_num; ++i)
        if (regcomp(&compiled_regex[i], regex_and_action[i][0], REG_EXTENDED)) {
            printf("regex error: %s\n", regex_and_action[i][0]);
            abort();
        }
    ...
    while (*pos != '\0') {
        ...
        for (i = 0; i < rule_num; ++i) {
            regmatch_t match;
            if (regexec(&compiled_regex[i], pos, 1, &match, 0) == 0) {
                int len = match.rm_eo;
